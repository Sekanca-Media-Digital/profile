...
<head>
    ...
    <style>
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .c { max-width: 720px; margin: 0 auto; padding: 2rem 1rem; }
        .block { display: grid; grid-template-columns: 1fr 1fr; gap: 2rem; align-items: center; margin-bottom: 3rem; }
        @media (max-width: 640px) { .block { grid-template-columns: 1fr; } }
        .block img { width: 100%; height: auto; border-radius: 8px; }
        .block h2 { font-size: 1.25rem; margin-bottom: 0.5rem; color: {{ $primary_color }}; }
        .block:nth-child(even) .img { order: 2; }
        .btns a { display: inline-block; padding: 0.5rem 1rem; background: {{ $primary_color }}; color: #fff; text-decoration: none; border-radius: 6px; margin-top: 0.75rem; margin-right: 0.5rem; }
...
<body>
    ...
    <div class="c">
        <div class="block">
            <div class="img">@if($image_url)<img src="{{ $image_url }}" alt="">@else<div style="background:#ddd;padding:50%;border-radius:8px;"></div>@endif</div>
            <div>
                <h2>{{ $headline }}</h2>
                <p>{{ $tagline }}</p>
                @if(count($buttons) > 0)
        ...
        <div class="block">
            <div><h2>Bagian kedua</h2><p>Deskripsi atau konten tambahan untuk blok kedua.</p></div>
            <div class="img"><div style="background:#e9ecef;padding:50%;border-radius:8px;"></div></div>
        </div>
    </div>
